...
registerPage("girlGenius", {
    ...
    createElements: () => {
        ...
        addFormattedParagraph("Way too late into development we were notified that Nintendo requires Addressables to be used in Unity games, in order to be approved for release on the Switch. Their reasoning is somewhat sound, as Unity normally creates its builds in the form of massive chunks of compressed binary data, which thanks to the compression will change completely if even just one string is changed. This is bad, because it means that even minor patches will be huge, which Nintendo doesn't want. Addressables allows developers to manually group their data into bundles, among other things. Unfortunately that also brings its own bag of issues with it, if not handled properly, such as assets, if used directly in multiple scenes, being duplicated, breaking reference equalities on one hand and ballooning build sizes on the other hand. ");
        addFormattedParagraph("We used the actual loading and more importantly unloading on demand of addressables only with the very large character dialogue sprites. Dealing with the async nature or the potential lag of forcing addressables to be synced was too much of a hassle for most of our scriptable objects, which were fairly light on the memory usage. To manage these in a non-reference-breaking way, we used \"configs\" (scriptable objects that contain other scriptable objects) and guids together with editor scripting for easy dropdowns. ");

        addSubHeader("Using the InputSystem");
        addFormattedParagraph("The input system was another neccessity, as there are very few good things that can be said about the built in input manager, except for that it is simple and generally works. As far as identifying controllers and providing easy access to a unified layout of them, it falls flat. The input system fixes this, but complicates everything else massively. Me and another programmer asked if we could just use ReWired instead, as that does the same thing, but has been out for a while and is very reliable, but no. ");
        addFormattedParagraph("Getting input from the input system isn't very difficult and neither is setting up gamepad bindings that work like you'd expect them to. Changing the binds at runtime is where the trouble lies. The documentation about rebinding is incorrect in places and leaves important things blank in others. Luckily with modern Unity packes, you get to take a look into the source code, so you can debug what counts as a 1.0 \"stable\" release yourself. After much sweat and tears (of joy, once it finally worked) however, full rebindability of the controls was achieved. And having used ReWired for a different project afterwards, I am more sure than ever that it would have been a far better choice. ");
        ...
        addFormattedParagraph("WttW had its own system for putting together cutscenes in the editor. The folks at Rain told me that one day some people from Unity came over to the office and saw their system, which may or may not have influenced the Timeline package. Unfortunately Timeline, as it is, was not usable for our cutscenes, which aren't the cinematic animated sequences that Timeline is made for, but rather a sequence of actions and waits which can't be timed beforehand. The best example is two characters walking to a place, then upon arriving starting a dialogue, which is driven by player input, after which they again walk away. ");
        ...
        addUnorderedList([
            ...
            "The stats and achievements system, tracking ingame stats and automatically triggering achievements upon certain goals being reached, among manual triggers of course",
            ...
            "The overarching system managing the grouping of all the addressables automatically",
            ...
            "The build-system and the grid upon it works and making it function with the only partially grid-based levels",
            "A custom ingame-menu system with custom navigation and all custom buttons, which turned out to be a very bad idea and a really poor idea once I had to add in sliders and input fields for the settings menu but it DOES work at least",
        ...
        addFormattedParagraph("I also got to do the AI for one enemy, the laser bot or \"Stingray\" as it's called in the game. They are a tad smarter than the exploding chicken bots, running away when you close in, chasing after you when you exit their field of fire and being very cute when they look around after chasing you, searching for their target. ");
