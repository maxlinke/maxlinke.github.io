...
registerPage("alchemist", {
    ...
    createElements: () => {
        ...
        addFormattedParagraph("During university I had used JavaFX for making GUIs for Java-applications. With .NET sometimes being called \"Microsoft Java\" I figured I might have a bit of a leg up with regards to building the user interface. And indeed, the general workflow was very similar. However the devil is of course in the details and I still had learn how to do everything in a framework I had never used before. ");
        addFormattedParagraph("While the Microsoft-Docs are expansive and very nice to read, finding what you're looking for when you don't know what it's called is a bit of a challenge. Case in point, making the tool to transform (move, resize, rotate) the text boxes involves an [Adorner](https://learn.microsoft.com/en-us/dotnet/desktop/wpf/controls/adorners-overview?view=netframeworkdesktop-4.8) which itself uses [Thumbs](https://learn.microsoft.com/en-us/dotnet/api/system.windows.controls.primitives.thumb?view=windowsdesktop-8.0) to handle the mouse interactions and I didn't know the terms for either one beforehand. Luckily, there were only a few instances of needing to know some magic word before being able to continue working. ");
        ...
        addFormattedParagraph("When working with windows applications, there are certain expectations about their layout and behaviour. I learned that at least with WPF, even though it literally stands for \"Windows Presentation Framework\", building a Windows-like interface and user experience required quite a bit of manual work. For example, perhaps unsurprisingly, undo and redo only function for certain premade elements, such as the text inside text boxes. So for everything else, a custom solution had to be used, for which I designed my own system. I also had a great time devising the file specification for our projects and implementing saving to and loading from disk. ");
    ...
    bitmap.Unlock();
        ...
        addFormattedParagraph("I wrote several algorithms for text removal. When using the automatic page-processing, we get a black and white text-mask from the text detector, but the mask can also be drawn manually. For maximum efficiency, this mask is actually a binary image in memory, which unfortunately also means that it has 0.125 bytes per pixel, since each byte contains the values for eight pixels, neccessitating bitshifting and binary operations in addition to the pointer magic. But once again, this wasn't my first time writing such code, so except for the occasional off-by-one error I didn't have much trouble. ");
