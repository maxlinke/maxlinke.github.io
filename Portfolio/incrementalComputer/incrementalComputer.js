...
void OnGUI () {
    ...
    if(curr.type == EventType.KeyDown){
        ...
        addFormattedParagraph("In the above example, the ||onKeyEvent|| observable is triggered every time a key is pressed. By subscribing to a filtered version of it, ||onCancel|| is triggered only if the the escape key was pressed. This can again be observed and filtered in other places, such as to close a menu, but only if the menu is open and focused, for example. ");
